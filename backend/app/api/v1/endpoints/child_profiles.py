...
async def create_child_profile(profile: ChildCreate, current_user=Depends(get_current_user), db=Depends(get_database)):
    ...
    children = await repo.get_child_profiles_by_user_id(current_user.id)
...
async def add_child_to_mother(mother_id, mother_list: list, db):
    user_repo = UserRepository(db)
    await user_repo.update_user_children(mother_id, mother_list)